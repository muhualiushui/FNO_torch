...
class ConditionUNet(nn.Module):
    """
    U-Net encoder/decoder for conditioning on the raw image.
    Produces hierarchical features to guide the diffusion model.
    """
    def __init__(self, in_channels=3, base_ch=64, depth=4):
        super().__init__()
        # Encoder
        ...
        for i in reversed(range(depth)):
            self.ups.append(nn.ConvTranspose2d(ch * 2, base_ch * (2**i), kernel_size=2, stride=2))
            self.ups.append(DoubleConv(ch * 2, base_ch * (2**i)))
            ch = base_ch * (2**i)
        # Final projection
...
class DiffusionUNet(nn.Module):
    """
    U-Net that takes (noisy_seg_map, conditioning_feats) and predicts the noise residual.
    """
    def __init__(self, in_channels=1, cond_channels=64, base_ch=64, depth=4):
        super().__init__()
        # The input has noisy map + image features concatenated
        self.init_conv = DoubleConv(in_channels + cond_channels, base_ch)
        # Encoder
        self.downs = nn.ModuleList()
        ch = base_ch
        ...
        for i in reversed(range(1, depth)):
            ...
            self.ups.append(DoubleConv(ch * 2, base_ch * (2**i)))
            ch = base_ch * (2**i)
        # Final conv to single-channel noise prediction
        self.out_conv = nn.Conv2d(base_ch, 1, kernel_size=1)

    def forward(self, noisy_map, cond_feats):
        # concatenate along channels
        x = torch.cat([noisy_map, cond_feats], dim=1)
        x = self.init_conv(x)
        skips = []
        for down in self.downs:
            skips.append(x)
