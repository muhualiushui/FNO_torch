...
class FNOnd(nn.Module):
    ...
    def train_epoch(self,
        ...
        running = 0.0
        total = 0
        pbar = tqdm(train_loader, desc='Train', leave=False, dynamic_ncols=False, position=0)
        for batch in pbar:
    ...
    def valid_epoch(self,
        ...
        total = 0
        with torch.no_grad():
            pbar = tqdm(test_loader, desc='Valid', leave=False, dynamic_ncols=False, position=0)
            for batch in pbar:
                # Support both tuple/list batches and dict batches keyed by x_name/y_name
                if isinstance(batch, dict):
                    if x_name is None or y_name is None:
