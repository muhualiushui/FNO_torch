...
from utils.visualization import select_slice,visualize
...
if __name__ == '__main__':
    ...
    transform2d = get_StrongAug(config.patch_size, 3, 0.7)
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        model.train()
        for batch_l in tqdm(train_loader):
        ...
        if epoch_num % 1 == 0:
            ...
            for step, batch in enumerate(tqdm(eval_loader)):
                with torch.no_grad():
                    ...
                    # print("image.shape:",image.shape,",gt.shape:",gt.shape,",p_u_theta.shape:",p_u_theta.shape)
                    if step%1000==0 and epoch_num%15==0:
                        visual_dict['image'] = select_slice(image,is_label =False)
                        visual_dict['label'] = select_slice(gt,is_label =True)
                        visual_dict['pred'] = select_slice(p_u_theta,is_label =True)
                        
                        vis_path_epoch = os.path.join(vis_path, f'epoch_{epoch_num}')
                        os.makedirs(vis_path_epoch,exist_ok=True)                        
                        visualize(
                            visual_dict,
                            save_path=os.path.join(vis_path_epoch, f'eval_{step}_result.png'),
